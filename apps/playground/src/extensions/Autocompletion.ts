      ...
      new Plugin({
          ...
          apply(tr, value, oldState, newState) {
            ...
            if (meta) {
              if (meta.type === 'setLoading') {
                ...
                if (!selection.empty) return { decoration: null, suggestion: null, isLoading: false };

                const ghostNode = document.createElement('span');
                ghostNode.className = 'inline-flex items-center gap-1 ml-1 pt-0.5 align-middle text-slate-300 pointer-events-none select-none';
                ghostNode.innerHTML = `
                  <span class="w-2.5 h-2.5 rounded-full bg-current animate-pulse" style="animation-duration: 1s; animation-delay: 0ms"></span>
                  <span class="w-2.5 h-2.5 rounded-full bg-current animate-pulse" style="animation-duration: 1s; animation-delay: 200ms"></span>
                  <span class="w-2.5 h-2.5 rounded-full bg-current animate-pulse" style="animation-duration: 1s; animation-delay: 400ms"></span>
                `;
              ...
              if (meta.type === 'setSuggestion') {
                ...
                const ghostNode = document.createElement('span');
                ghostNode.className = 'text-slate-400 opacity-60 pointer-events-none select-none';
                ...
                return { decoration: dec, suggestion: meta.suggestion, isLoading: false };
              }
              if (meta.type === 'clear') {
                 return { decoration: null, suggestion: null, isLoading: false };
              }
        ...
        props: {
          decorations(state) {
            ...
            return pluginState?.decoration ? DecorationSet.create(state.doc, [pluginState.decoration]) : DecorationSet.empty;
          },
          handleKeyDown(view, event) {
            ...
            if ((pluginState?.suggestion || pluginState?.isLoading) && event.key === 'Escape') {
              view.dispatch(view.state.tr.setMeta(AutocompletionKey, { type: 'clear' }));
              return true;
            }
        ...
        view() {
          return {
            update(view) {
              ...
              const trailing = state.doc.textBetween(from, Math.min(state.doc.content.size, from + 10), ' ');
              ...
              if (cached !== undefined) {
                ...
                if (cached) {
                  view.dispatch(view.state.tr.setMeta(AutocompletionKey, { type: 'setSuggestion', suggestion: cached }));
                }
                return;
              }
              ...
              timeout = setTimeout(() => {
                ...
                if (!currentSelection.empty || currentSelection.from !== from) return;

                view.dispatch(view.state.tr.setMeta(AutocompletionKey, { type: 'setLoading' }));
                ...
                fetchSuggestion(text).then((suggestion) => {
                  ...
                  if (!stillCurrentSelection.empty || stillCurrentSelection.from !== from) return;

                  let cleanSuggestion = suggestion.replace(/^(\n| )+/, ' ');
                  ...
                  if (cleanSuggestion) {
                     view.dispatch(view.state.tr.setMeta(AutocompletionKey, { type: 'setSuggestion', suggestion: cleanSuggestion }));
                  } else {
                     view.dispatch(view.state.tr.setMeta(AutocompletionKey, { type: 'clear' }));
                  }
                }).catch((err: any) => {
                  // Any API error → enter cooldown to stop hammering the API
                  const errBody = typeof err?.message === 'string' ? err.message : '';
                  const isQuotaError =
                    err?.code === 429 ||
                    err?.status === 429 ||
                  ...
                  if (isQuotaError) {
                    console.warn('[Autocompletion] Quota exceeded, cooling down', QUOTA_COOLDOWN_MS / 1000, 's');
                  } else {
                    console.error('[Autocompletion] Error, cooling down', err);
                  }
                });
              }, DEBOUNCE_MS);
            },
