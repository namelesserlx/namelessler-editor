        ...
        <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
            <DialogContent className="w-[90vw] max-w-5xl sm:max-w-5xl md:max-w-6xl flex flex-col h-[85vh] p-0 gap-0 border-slate-200">
                ...
                <Tabs
                    ...
                    value={activeTab}
                    onValueChange={(val) => setActiveTab(val as 'markdown' | 'json' | 'html')}
                ...
                >
                    <div className="flex items-center justify-between p-4 border-b border-slate-100 bg-slate-50/50">
                        <TabsList className="bg-slate-100/80 p-1">
                            <TabsTrigger
                                value="markdown"
                                className="flex items-center space-x-1.5 px-3 py-1.5 text-sm data-[state=active]:bg-white data-[state=active]:text-slate-800"
                            >
